...
- [ ] 写真削除: `ConfirmModal`で確認 → S3の削除APIを呼び出し、成功した場合のみローカルからも削除する。失敗時はローカルの状態を変えずエラーを表示し、再度削除を試せるようにする。既存のUndo(スナックバーの取り消し)は廃止する。削除APIも`adminS3Context.ts`経由で注入する
